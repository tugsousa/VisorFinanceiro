...
import { optionSalesColumns } from '../config/salesGridConfig';
...
export default function OptionSalesSection({ optionSalesData, selectedYear, isLoading, NoRowsOverlay }) {
    const { productChartData, timeSeriesChartData } = useMemo(() => 
        prepareSalesCharts(optionSalesData, selectedYear, { productName: 'product_name', pl: 'delta', date: 'close_date' }), 
            ...
            {isLoading && !hasData ? (
                ...
                <>
                    ...
                    <Box sx={{ width: '100%' }}>
                        <DataGrid
                            rows={rows}
                            columns={optionSalesColumns}
                            loading={isLoading}
                            autoHeight
                            initialState={{
                                pagination: { paginationModel: { pageSize: 10 } },
                                sorting: { sortModel: [{ field: 'open_date', sort: 'desc' }] },
                            }}
